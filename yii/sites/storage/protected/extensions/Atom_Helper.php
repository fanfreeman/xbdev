...
class Atom_Helper {
	...
	public static function linkTo( $app, $file, $redirect_url=NULL, $absolute=false ){
		if( $app != '' ){
			...
			if( file_exists( Atom::$SERVERROOT . '/apps/'. $app.$file )){
				$urlLinkTo =  Atom::$WEBROOT . '/apps/' . $app . $file;
			}
			else{
				$urlLinkTo =  Atom::$WEBROOT . '/' . $app . $file;
			}
		}
		else{
			if( file_exists( Atom::$SERVERROOT . '/core/'. $file )){
				$urlLinkTo =  Atom::$WEBROOT . '/core/'.$file;
			}
			else{
				$urlLinkTo =  Atom::$WEBROOT . '/'.$file;
			}
		}
	...
	public static function imagePath( $app, $image ){
		// Check if the app is in the app folder
		if( file_exists( Atom::$SERVERROOT."/apps/$app/img/$image" )){
			return Atom::$WEBROOT."/apps/$app/img/$image";
		}
		elseif( !empty( $app )){
			return Atom::$WEBROOT."/$app/img/$image";
		}
		else{
			return Atom::$WEBROOT."/core/img/$image";
		}
	}
...
// 			echo $mimetype;
		...
		// Is it a dir?
		if( $mimetype == "dir" ){
			return Atom::$WEBROOT."/core/img/filetypes/folder.png";
		}

		// Icon exists?
		if( file_exists( Atom::$SERVERROOT."/core/img/filetypes/$mimetype.png" )){
			return Atom::$WEBROOT."/core/img/filetypes/$mimetype.png";
		}
		...
		if( file_exists( Atom::$SERVERROOT."/core/img/filetypes/$mimetype.png" )){
			...
			return Atom::$WEBROOT."/core/img/filetypes/file.png";
		}
	}
