...
class Atom_Group {
	...
	public static function setBackend( $backend = 'database' ){
		...
		switch( $backend ){
			...
			case 'mysql':
			case 'sqlite':
				self::$_backend = new Atom_Group_Database();
				break;
			default:
				$className = 'Atom_GROUP_' . strToUpper($backend);
	...
	public static function createGroup( $gid ){
		...
		$run = true;
		Atom_Hook::emit( "Atom_Group", "pre_createGroup", array( "run" => &$run, "gid" => $gid ));

		if( $run && self::$_backend->createGroup( $gid )){
			Atom_Hook::emit( "Atom_Group", "post_createGroup", array( "gid" => $gid ));
			return true;
		}
		else{
			return false;
	...
	public static function deleteGroup( $gid ){
		...
		$run = true;
		Atom_Hook::emit( "Atom_Group", "pre_deleteGroup", array( "run" => &$run, "gid" => $gid ));

		if( $run && self::$_backend->deleteGroup( $gid )){
			Atom_Hook::emit( "Atom_Group", "post_deleteGroup", array( "gid" => $gid ));
			return true;
		}
		else{
			return false;
	...
	public static function addToGroup( $uid, $gid ){
		// Does the user exist?
		if( !Atom_User::userExists($uid)){
			return false;
		}
		// Does the group exist?
		if( !Atom_Group::groupExists($gid)){
			return false;
		}

		// Go go go
		$run = true;
		Atom_Hook::emit( "Atom_Group", "pre_addToGroup", array( "run" => &$run, "uid" => $uid, "gid" => $gid ));

		if( $run && self::$_backend->addToGroup( $uid, $gid )){
			Atom_Hook::emit( "Atom_Group", "post_addToGroup", array( "uid" => $uid, "gid" => $gid ));
			return true;
		}
		else{
			return false;
	...
	public static function removeFromGroup( $uid, $gid ){
		$run = true;
		Atom_Hook::emit( "Atom_Group", "pre_removeFromGroup", array( "run" => &$run, "uid" => $uid, "gid" => $gid ));

		if( $run && self::$_backend->removeFromGroup( $uid, $gid )){
			Atom_Hook::emit( "Atom_Group", "post_removeFromGroup", array( "uid" => $uid, "gid" => $gid ));
			return true;
		}
		else{
			return false;
