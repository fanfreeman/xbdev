...
class Atom_DB {
	...
	public static function connect($backend=null){
		...
		if(is_null($backend)){
			...
			if(class_exists('PDO') && Atom_Config::getValue('installed', false)){//check if we can use PDO, else use MDB2 (instalation always needs to be done my mdb2)
				$type = Atom_Config::getValue( "dbtype", "sqlite" );
	...
	private static function connectPDO(){
		...
		$name = Atom_Config::getValue( "dbname", "owncloud" );
		$host = Atom_Config::getValue( "dbhost", "" );
		$user = Atom_Config::getValue( "dbuser", "" );
		$pass = Atom_Config::getValue( "dbpassword", "" );
		$type = Atom_Config::getValue( "dbtype", "sqlite" );
		$datadir=Atom_Config::getValue( "datadirectory", Atom::$SERVERROOT.'/data' );
	...
	static private function connectMDB2(){
		...
		$pass = Atom_Config::getValue( "dbpassword", "" );
		$type = Atom_Config::getValue( "dbtype", "sqlite" );
		$SERVERROOT=Atom::$SERVERROOT;
		$datadir=Atom_Config::getValue( "datadirectory", "$SERVERROOT/data" );
		...
		if(!self::$MDB2){
			...
			if( PEAR::isError( self::$MDB2 )){
				...
				Atom_Log::write('core',self::$MDB2->getUserInfo(),Atom_Log::FATAL);
				Atom_Log::write('core',self::$MDB2->getMessage(),Atom_Log::FATAL);
				die( $error );
			}
	...
	static public function prepare( $query ){
		...
		if(self::$backend==self::BACKEND_MDB2){
			...
			if( PEAR::isError($result)) {
				...
				Atom_Log::write('core',$entry,Atom_Log::FATAL);
				die( $entry );
			}
		}else{
			try{
	...
	public static function insertid($table=null){
		self::connect();
		if($table !== null){
			$prefix = Atom_Config::getValue( "dbtableprefix", atom_" );
			$suffix = Atom_Config::getValue( "dbsequencesuffix", "_id_seq" );
	...
	public static function createDbFromStructure( $file ){
		...
		$CONFIG_DBTABLEPREFIX = Atom_Config::getValue( "dbtableprefix", "atom_" );
		...
		$file2 = tempnam( get_temp_dir(), 'atom_db_scheme_' );
...
// 		if(Atom_Config::getValue('dbtype','sqlite')=='sqlite'){
...
// 		}
	...
	public static function updateDbFromStructure($file){
		...
		if (PEAR::isError($op)) {
		    $error = $op->getMessage();
		    Atom_Log::write('core','Failed to update database structure ('.$error.')',Atom_Log::FATAL);
		    return false;
		}
		return true;
	}
...
class PDOStatementWrapper{
	...
	public function numRows(){
		...
		if (preg_match($regex, $this->statement->queryString, $output) > 0) {
			$query = Atom_DB::prepare("SELECT COUNT(*) FROM {$output[1]}", PDO::FETCH_NUM);
