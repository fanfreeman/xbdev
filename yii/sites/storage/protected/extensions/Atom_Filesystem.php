...
* This class won't call any filesystem functions for itself but will pass them to the correct Atom_Filestorage object
...
class Atom_Filesystem{
	...
	/**
	 * mount an Atom_Filestorage in our virtual filesystem
	 * @param Atom_Filestorage storage
	...
	static public function rename($path1,$path2){
		if(Atom_FileProxy::runPreProxies('rename',$path1,$path2) and self::canWrite($path1) and self::canWrite($path2)){
			$run=true;
			Atom_Hook::emit( 'Atom_Filesystem', 'rename', array( 'oldpath' => $path1 ,'newpath'=>$path2, 'run' => &$run));
			if($run){
				...
				Atom_Hook::emit( 'Atom_Filesystem', 'post_rename', array( 'oldpath' => $path1, 'newpath'=>$path2));
				return $result;
			}
		}
	}
	static public function copy($path1,$path2){
		if(Atom_FileProxy::runPreProxies('copy',$path1,$path2) and self::canRead($path1) and self::canWrite($path2)){
			$run=true;
			Atom_Hook::emit( 'Atom_Filesystem', 'copy', array( 'oldpath' => $path1 ,'newpath'=>$path2, 'run' => &$run));
			$exists=self::file_exists($path2);
			if($run and !$exists){
				Atom_Hook::emit( 'Atom_Filesystem', 'create', array( 'path' => $path2, 'run' => &$run));
			}
			if($run){
				Atom_Hook::emit( 'Atom_Filesystem', 'write', array( 'path' => $path2, 'run' => &$run));
			}
			if($run){
				$mp1=self::getMountPoint($path1);
				$mp2=self::getMountPoint($path2);
				...
				Atom_Hook::emit( 'Atom_Filesystem', 'post_copy', array( 'oldpath' => $path1 ,'newpath'=>$path2));
				if(!$exists){
					Atom_Hook::emit( 'Atom_Filesystem', 'post_create', array( 'path' => $path2));
				}
				Atom_Hook::emit( 'Atom_Filesystem', 'post_write', array( 'path' => $path2));
				return $result;
			}
		}
	}
	static public function fopen($path,$mode){
		...
		switch($mode){
			...
			case 'a':
				...
				break;
			default:
				Atom_Log::write('core','invalid mode ('.$mode.') for '.$path,Atom_Log::ERROR);
		}

		return self::basicOperation('fopen',$path,$hooks,$mode);
	}
	static public function toTmpFile($path){
		if(Atom_FileProxy::runPreProxies('toTmpFile',$path) and self::canRead($path) and $storage=self::getStorage($path)){
			Atom_Hook::emit( 'Atom_Filesystem', 'read', array( 'path' => $path));
			return $storage->toTmpFile(self::getInternalPath($path));
		}
	}
	static public function fromTmpFile($tmpFile,$path){
		if(Atom_FileProxy::runPreProxies('copy',$tmpFile,$path) and self::canWrite($path) and $storage=self::getStorage($path)){
			$run=true;
			$exists=self::file_exists($path);
			if(!$exists){
				Atom_Hook::emit( 'Atom_Filesystem', 'create', array( 'path' => $path, 'run' => &$run));
			}
			if($run){
				Atom_Hook::emit( 'Atom_Filesystem', 'write', array( 'path' => $path, 'run' => &$run));
			}
			if($run){
				$result=$storage->fromTmpFile($tmpFile,self::getInternalPath($path));
				if(!$exists){
					Atom_Hook::emit( 'Atom_Filesystem', 'post_create', array( 'path' => $path));
				}
				Atom_Hook::emit( 'Atom_Filesystem', 'post_write', array( 'path' => $path));
				return $result;
			}
		}
	}
	static public function fromUploadedFile($tmpFile,$path){
		if(Atom_FileProxy::runPreProxies('fromUploadedFile',$tmpFile,$path) and self::canWrite($path) and $storage=self::getStorage($path)){
			$run=true;
			$exists=self::file_exists($path);
			if(!$exists){
				Atom_Hook::emit( 'Atom_Filesystem', 'create', array( 'path' => $path, 'run' => &$run));
	...
	private static function basicOperation($operation,$path,$hooks=array(),$extraParam=null){
		if(Atom_FileProxy::runPreProxies($operation,$path, $extraParam) and self::canRead($path) and $storage=self::getStorage($path)){
			
			$interalPath=self::getInternalPath($path);
			$run=true;
			foreach($hooks as $hook){
				if($hook!='read'){
					Atom_Hook::emit( 'Atom_Filesystem', $hook, array( 'path' => $path, 'run' => &$run));
				}else{
					Atom_Hook::emit( 'Atom_Filesystem', $hook, array( 'path' => $path));
				}
			}
			if($run){
				if($extraParam){
				...
				$result=Atom_FileProxy::runPostProxies($operation,$path,$result);
				foreach($hooks as $hook){
					if($hook!='read'){
						Atom_Hook::emit( 'Atom_Filesystem', 'post_'.$hook, array( 'path' => $path));
					}
				}
				return $result;
			}
