...
class Messages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('message_id, sender_id, message_group_id, message, created', 'required'),
			
            array('message_id, sender_id, message_group_id, created', 'length', 'max'=>10),
			
            array('message', 'length', 'max'=>140),
			
            // The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('message_id, sender_id, message_group_id, message, created', 'safe', 'on'=>'search'),
		);
	}
