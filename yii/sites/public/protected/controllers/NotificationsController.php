...
class NotificationsController extends Controller
{
	...
	*/
	 ...
	 * Ting Yee
	 */
	public function actionTinge($method = '', $page = 1)
	{
		if ($method == 'add') {
			if(bizNotifications::add('1', '3', '111'))
				exit('添加成功');
			else
				exit('添加失败');
		}elseif($method == 'delete') {
			if(bizNotifications::delete('1', '4'))
				exit('删除成功');
			else
				exit('删除失败');
		}else {
			$intUser_id = 1;
			$arrayList = bizNotifications::getList($intUser_id, $page);
			foreach ($arrayList['data'] as $key => $val) {
				echo "user_id:" . $val['user_id'] . "\t" . $val['resource_type'] . "<br />";
			}
    ...
    {	
		...
		$list_notifications = bizNotifications::getList($user_id);
		$this->renderPartial('notifications',$list_notifications);
	}
	
	public function actionLoadCheckNotification()
	{
		$this->renderPartial('timeout');
	}		
}
