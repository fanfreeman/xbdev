 ...
 */
 $(document).ready(function() {
	...
	$("#informationRecorded").atomPageAuto({ type : "width", num : 236 });
	//调整右侧senderShow宽度	
...
function loadAtomMessages(all_members,other_members,message_group_id,last_message_id){
	messageAjax('loadAtomMessages','get',"index.php?r=Message/loadMessages",{all_members : all_members,other_members : other_members,message_group_id:message_group_id,last_message_id:last_message_id});
...
$.ajax({
			type : 'get',
			url : "index.php?r=Message/LoadCheckMessage",
			data:'',
			dataType : "json",
			cache : true,
					...
					if(json.has_data)
					{
						...
						if($('#li_chatgroup_'+message_group_id).length)
						{
							//if(!array_message_group_ids.in_array(message_group_id))
							{
								...
								{
									messageAjax('GetNoreadMessage','get',"index.php?r=Message/GetNoreadMessage",{message_group_id : message_group_id,last_message_id:last_message_id});
								}
								else
								{
						...
						{
						//没有左侧对话框，添加对话框
						messageAjax('LoadNewMessageGroup','get',"index.php?r=Message/LoadNewMessageGroup",{message_group_id : message_group_id});
						}
					}
					else
					{
